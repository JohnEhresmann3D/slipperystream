...
impl CollisionGrid {
    /// Compatibility constructor for trusted code-authored grids. External/generated
    /// data should use `try_from_file` or `parse_collision` to receive validation errors.
    pub fn from_file(file: CollisionFile) -> Self {
        Self::try_from_file(file).expect("Invalid collision grid")
    }

    pub fn try_from_file(file: CollisionFile) -> Result<Self, String> {
...
pub fn load_collision_from_path(path: &Path) -> Result<CollisionGrid, String> {
    ...
    parse_collision(raw.as_bytes()).map_err(|e| format!("{}: {e}", path.display()))
}

/// Portable parser with validated grid construction.
pub fn parse_collision(bytes: &[u8]) -> Result<CollisionGrid, String> {
    CollisionGrid::try_from_file(crate::asset_data::parse_json(bytes, "collision")?)
}
