...
use crate::animation::{load_animation_file, AnimationClip, AnimationFile};
...
impl AnimationRegistry {
    ...
    pub fn load_file(&mut self, path: &Path) -> Result<(), String> {
        self.add_file(load_animation_file(path)?)
    }

    /// Register already-parsed data, including data fetched by a browser host.
    /// Duplicate IDs are rejected so reload candidates cannot silently lose clips.
    pub fn add_file(&mut self, file: AnimationFile) -> Result<(), String> {
        if self.clips.contains_key(&file.animation_id) {
            return Err(format!("Duplicate animation_id '{}'", file.animation_id));
        }
