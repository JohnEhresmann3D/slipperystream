...
pub fn load_atlas_from_path(path: &Path) -> Result<AtlasRegistry, String> {
    ...
    parse_atlas(raw.as_bytes()).map_err(|e| format!("{}: {e}", path.display()))
}

/// Parse embedded/fetched atlas metadata; texture acquisition belongs to the host.
pub fn parse_atlas(bytes: &[u8]) -> Result<AtlasRegistry, String> {
    let atlas: AtlasFile = crate::asset_data::parse_json(bytes, "atlas")?;
...
impl MultiAtlasRegistry {
    ...
    pub fn add_atlas(&mut self, key: &str, registry: AtlasRegistry) -> Result<(), String> {
        if self.registries.contains_key(key) {
            return Err(format!(
                "Atlas key '{key}' already exists; use a staged replacement"
            ));
        }
    ...
    /// Build a replacement without modifying the live registry. The caller must
    /// validate scene/animation references and stage textures before committing it.
    pub fn with_replacement(&self, key: &str, registry: AtlasRegistry) -> Result<Self, String> {
        let mut candidate = self.clone();
        candidate.remove_atlas(key);
        candidate.add_atlas(key, registry)?;
        Ok(candidate)
    }
...
mod tests {
    ...
    #[test]
    fn rejected_replacement_preserves_live_registry_and_same_key_cannot_leak_ids() {
        let mut live = MultiAtlasRegistry::new();
        live.add_atlas("a", make_test_registry("a", &[("player", "old.png")]))
            .unwrap();
        live.add_atlas("b", make_test_registry("b", &[("enemy", "enemy.png")]))
            .unwrap();
        assert!(live
            .with_replacement("a", make_test_registry("bad", &[("enemy", "bad.png")]))
            .is_err());
        assert_eq!(live.resolve("player").unwrap().texture_path, "old.png");
        assert_eq!(live.atlas_count(), 2);
        assert!(live
            .add_atlas("a", make_test_registry("new", &[("other", "new.png")]))
            .is_err());
        assert!(live.resolve("other").is_none());
        let next = live
            .with_replacement("a", make_test_registry("a", &[("player", "new.png")]))
            .unwrap();
        assert_eq!(next.resolve("player").unwrap().texture_path, "new.png");
        assert_eq!(live.resolve("player").unwrap().texture_path, "old.png");
    }
