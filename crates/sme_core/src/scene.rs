...
pub fn load_scene_from_path(scene_path: &Path) -> Result<SceneFile, String> {
    ...
    parse_scene(raw.as_bytes()).map_err(|e| format!("{}: {e}", scene_path.display()))
}

/// Parse embedded/fetched bytes with the same validation as native file loading.
pub fn parse_scene(bytes: &[u8]) -> Result<SceneFile, String> {
    let scene: SceneFile = crate::asset_data::parse_json(bytes, "scene")?;
